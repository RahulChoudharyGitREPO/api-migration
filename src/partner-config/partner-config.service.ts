...
import { PartnerConfig, PartnerConfigSchema } from './partner-config.schema';
...
export class PartnerConfigService {
  private getPartnerConfigModel(dbConnection: Connection): Model<PartnerConfig> {
    // Register User model for population
    if (!dbConnection.models['users']) {
      dbConnection.model('users', UserSchema, 'users');
    }
    return dbConnection.model<PartnerConfig>('partnerconfigurations', PartnerConfigSchema, 'partnerconfigurations');
  }

  async create(dbConnection: Connection, createDto: CreatePartnerConfigDto, userId: string): Promise<any> {
    const PartnerConfigModel = this.getPartnerConfigModel(dbConnection);
    ...
    const result = await PartnerConfigModel.create({
      ...createDto,
      createdBy: userId,
      updatedBy: userId,
    });
  ...
  async update(dbConnection: Connection, updateDto: UpdatePartnerConfigDto, userId: string): Promise<any> {
    const PartnerConfigModel = this.getPartnerConfigModel(dbConnection);
    ...
    const result = await PartnerConfigModel.findByIdAndUpdate(
      updateDto.id,
    ...
    if (!result) {
      throw new NotFoundException('PartnerConfig not found!');
    }

    return { success: true, data: result };
  }

  async list(dbConnection: Connection, listDto: ListPartnerConfigDto): Promise<any> {
    const PartnerConfigModel = this.getPartnerConfigModel(dbConnection);
    const {
      page = 1,
      limit = 10,
    ...
    const countResult = await PartnerConfigModel.aggregate(totalCountPipeline);
    ...
    const result = await PartnerConfigModel.aggregate([
      ...query,
      { $sort: sorts },
      { $skip: skip },
      { $limit: limit },
  ...
  async getDetails(dbConnection: Connection, id: string): Promise<any> {
    ...
    const result = await PartnerConfigModel.findById(id).populate(['createdBy', 'updatedBy']);
