...
import { ProgramConfig, ProgramConfigSchema } from './program-config.schema';
...
export class ProgramConfigService {
  private getProgramConfigModel(dbConnection: Connection): Model<ProgramConfig> {
    // Register User model for population
    if (!dbConnection.models['users']) {
      dbConnection.model('users', UserSchema, 'users');
    }
    return dbConnection.model<ProgramConfig>('programconfigurations', ProgramConfigSchema, 'programconfigurations');
  }

  async create(dbConnection: Connection, createDto: CreateProgramConfigDto, userId: string): Promise<any> {
    const ProgramConfigModel = this.getProgramConfigModel(dbConnection);
    ...
    const result = await ProgramConfigModel.create({
      ...createDto,
      createdBy: userId,
      updatedBy: userId,
    });
  ...
  async update(dbConnection: Connection, updateDto: UpdateProgramConfigDto, userId: string): Promise<any> {
    const ProgramConfigModel = this.getProgramConfigModel(dbConnection);
    ...
    const result = await ProgramConfigModel.findByIdAndUpdate(
      updateDto.id,
    ...
    if (!result) {
      throw new NotFoundException('ProgramConfig not found!');
    }

    return { success: true, data: result };
  }

  async list(dbConnection: Connection, listDto: ListProgramConfigDto): Promise<any> {
    const ProgramConfigModel = this.getProgramConfigModel(dbConnection);
    const {
      page = 1,
      limit = 10,
    ...
    const countResult = await ProgramConfigModel.aggregate(totalCountPipeline);
    ...
    const result = await ProgramConfigModel.aggregate([
      ...query,
      { $sort: sorts },
      { $skip: skip },
      { $limit: limit },
  ...
  async getDetails(dbConnection: Connection, id: string): Promise<any> {
    ...
    const result = await ProgramConfigModel.findById(id).populate(['createdBy', 'updatedBy']);
