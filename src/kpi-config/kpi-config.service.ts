...
import { KpiConfig, KpiConfigSchema } from './kpi-config.schema';
...
export class KpiConfigService {
  private getKpiConfigModel(dbConnection: Connection): Model<KpiConfig> {
    // Register User model for population
    if (!dbConnection.models['users']) {
      dbConnection.model('users', UserSchema, 'users');
    }
    return dbConnection.model<KpiConfig>('kpiconfigurations', KpiConfigSchema, 'kpiconfigurations');
  }

  async create(dbConnection: Connection, createDto: CreateKpiConfigDto, userId: string): Promise<any> {
    const KpiConfigModel = this.getKpiConfigModel(dbConnection);
    ...
    const result = await KpiConfigModel.create({
      ...createDto,
      createdBy: userId,
      updatedBy: userId,
    });
  ...
  async update(dbConnection: Connection, updateDto: UpdateKpiConfigDto, userId: string): Promise<any> {
    const KpiConfigModel = this.getKpiConfigModel(dbConnection);
    ...
    const result = await KpiConfigModel.findByIdAndUpdate(
      updateDto.id,
    ...
    if (!result) {
      throw new NotFoundException('KpiConfig not found!');
    }

    return { success: true, data: result };
  }

  async list(dbConnection: Connection, listDto: ListKpiConfigDto): Promise<any> {
    const KpiConfigModel = this.getKpiConfigModel(dbConnection);
    const {
      page = 1,
      limit = 10,
    ...
    const countResult = await KpiConfigModel.aggregate(totalCountPipeline);
    ...
    const result = await KpiConfigModel.aggregate([
      ...query,
      { $sort: sorts },
      { $skip: skip },
      { $limit: limit },
  ...
  async getDetails(dbConnection: Connection, id: string): Promise<any> {
    ...
    const result = await KpiConfigModel.findById(id).populate(['createdBy', 'updatedBy']);
